...
(*
  ...
  ***** END LICENSE BLOCK *****
  ...
  Version 1.18
  - renamed SQLite3UIEdit.pas to UIEdit.pas
...
{$endif}
  SynCommons, SynTable,
  mORMot, mORMotUILogin, mORMotUI, mORMoti18n, mORMotToolBar,
    ...
    for i := 0 to Fields.Count-1 do begin
      ...
      if C=nil then begin
        ...
        case aFieldType of
          ...
          sftUTF8Text, sftAnsiText: begin
            ...
            CLE.Anchors := [akLeft, akTop, akRight];
            CLE.Text := U2S(aValue);
          end;
        end;
      end;
      if (C<>nil) and (C<>self) and (C<>Scroll) then begin
        ...
        if C.InheritsFrom(TDateTimePicker) then begin
          ...
          C.SetBounds(FieldNamesWidth,Y,250,22);
  ...
  for FieldIndex := 0 to high(fFieldComponents) do begin
    ...
    if C.InheritsFrom(TCheckBox) then begin
      if CC.Tag<255 then begin
        ...
        P.SetValue(Rec, Pointer(StringToUTF8(BoolToStr(CC.Checked))),False);
        //
        Include(ModifiedFields,FieldIndex);
      end;
    end else
