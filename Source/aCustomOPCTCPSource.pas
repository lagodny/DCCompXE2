...
uses
  ...
  //aCustomOPCSource,
  aOPCSource,
  IdTCPClient, IdGlobal, IdException, IdExceptionCore, SyncObjs
  , IdStackConsts
...
//    property Language: TUserLanguage read FLanguage write SetLanguage default langRU;
    ...
    // альтернативные настройки подключени€ будут использоватьс€,
...
begin
  // все длительные операции в потоке св€заны с получением данных через TCP
  // такие операции выполн€ютс€ в критической секции
...
begin
  TestMsg('ReadLn');
  try
    Result := FConnection.IOHandler.ReadLn(EOL, ReadTimeOut);
    if FConnection.IOHandler.ReadLnTimedout then
    begin
      TestMsg('ReadLn ReadLnTimedout.');
      raise EIdReadTimeout.Create('ReadLn timeout');
    end
    else
      TestMsg('ReadLn OK.');
  except
    on e:Exception do
      TestMsg('ReadLn Exception: ' + e.Message);
  end;
end;
...
var
  TickCount1: Cardinal;
begin
  TickCount1 := Windows.GetTickCount;
  try
    TryConnect;
  except
    try
  ...
  if UpdateMode = umAuto then
    PacketUpdate := ((Windows.GetTickCount - TickCount1) > 1000);
...
begin
  ...
  {
    альтернативные подключени€ задаютс€ строкой вида:
