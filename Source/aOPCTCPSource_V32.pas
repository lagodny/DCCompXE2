...
type
  ...
  public
    ...
    function GetProjectVersionUserFileName(aProjectID, aVersionID: Integer): string;
    ...
    function AddProjectVersion(aProjectID: Integer;
      aLabel, aComment, aDescription, aAuthor, aUserFileName: string;
...
function TaOPCTCPSource_V32.AddProjectVersion(aProjectID: Integer; aLabel,
  aComment, aDescription, aAuthor, aUserFileName: string; aStream: TStream;
...
begin
  Assert(Assigned(aStream), GetStringRes(idxStream_NotCreated));
...
// попытка 2
{
  ...
  begin
    ...
    raise EOPCTCPOperationCanceledException.Create(
      GetStringRes(idxTCPCommand_OperationCanceledByUser));
  end;
...
function TaOPCTCPSource_V32.GetProjectVersionUserFileName(aProjectID, aVersionID: Integer): string;
begin
  Result := LockDoCommandReadLnFmt('GetProjectVersionUserFileName %d;%d', [aProjectID, aVersionID]);
end;
