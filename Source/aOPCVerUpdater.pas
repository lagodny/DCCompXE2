...
uses
  uDCLang, uDCLocalizer, uDCStatus, uDCCommonProc,
  SysUtils,
  JclFileUtils,
  JvVersionInfo;
...
//    MB_YESNO + MB_ICONINFORMATION);
  ...
  if MessageBox(
    ...
    PChar(Format(TDCLocalizer.GetStringRes(idxUpdate_Available), [aVer2])),
  ...
  try
    ...
    DCStatus.ShowStatus(Format(
      TDCLocalizer.GetStringRes(idxUpdate_SetupDownloading),
  ...
  begin
    if ConfirmRun then
      if MessageBox(
        Application.Handle,
        PChar(Format(TDCLocalizer.GetStringRes(idxUpdate_SetupSuccesfullyLoaded), [#13#10 + aFileName + #13#10])),
        PChar(Application.Title),
        MB_YESNO + MB_ICONQUESTION) <> mrYes then
        Exit;
    ...
    MessageBox(
      Application.Handle,
      PChar(TDCLocalizer.GetStringRes(idxUpdate_UnableDownloadNewVersion)),
      PChar(Application.Title),
      MB_OK + MB_ICONERROR);
