...
type
  ...
  public
    ...
    //после того, как данные прочитаны (вызываетс€ в потоке)
    property OnChangeDataThreaded: TNotifyEvent read FOnChangeDataThreaded write FOnChangeDataThreaded;
    //после того, как все данные почитаны (вызываетс€ в главном потоке)
...
begin
  ...
  // поиск может выполн€тьс€ в основном потоке приложени€
  // необходимо запретить потоку сбора данных ”паковать
...
begin
  ...
  begin
    ...
    // удал€ем группу, если ƒатаЋанков не осталось
