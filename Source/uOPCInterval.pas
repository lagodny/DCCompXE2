...
//        Result := TDCLocalizer.GetStringRes(idxInterval_skNextDay); //'Седующий день';
  ...
  begin
    if TimeShiftUnit = tsuHour then
      Result := Format(TDCLocalizer.GetStringRes(idxInterval_LastHours), //'последние %s ч.',
        [FormatFloat('0.##', TimeShift * 24)])
    else
      Result := Format(TDCLocalizer.GetStringRes(idxInterval_LastDays), //'последние %s д.',
...
begin
  case aShiftKind of
    ...
    skNextDay:
      Result := TDCLocalizer.GetStringRes(idxInterval_skNextDay); //'Седующий день';
    skYear:
      Result := TDCLocalizer.GetStringRes(idxInterval_skYear); //'Этот год';
    skLastYear:
      Result := TDCLocalizer.GetStringRes(idxInterval_skLastYear); //'Прошлый год';
