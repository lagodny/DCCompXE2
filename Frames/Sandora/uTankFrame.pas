...
uses
  ...
  uOPCFrame, aOPCDataObject, aOPCImage, aOPCImage2In, aOPCGifImage, aOPCImageList,
  aOPCStateLine;
...
type
  TfrTank = class(TaOPCFrame)
    ...
    MassaGif: TaOPCGifImage;
    TempGif: TaOPCGifImage;
    lTKCaption: TaOPCLabel;
    lTK: TaOPCLabel;
    CoolSystem: TShape;
    Pump: TaOPCGifImage;
    Mixer: TaOPCImage2In;
    Cooling: TaOPCGifImage;
...
procedure TfrTank.LocalInit(aId: integer; aOPCObjects: TOPCObjectList);
var
...
begin
  if MixerStateLine.Interval < 1 then
    MixerStateLine.Interval := 12
  else
    MixerStateLine.Interval := MixerStateLine.Interval/10;

  if MixerStateLine.Interval = 12 then
    MixerStateLine.Height := 6
  else if Round(MixerStateLine.Interval*10) = 12 then
    MixerStateLine.Height := 5
  else if Round(MixerStateLine.Interval*100) = 12 then
    MixerStateLine.Height := 4;


    
  MixerStateLine.Hint := Format('Работа мешалки за последние %s ч.',[FormatFloat('0.##',MixerStateLine.Interval)]);
