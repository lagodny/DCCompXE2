...
type
  ...
  published
    property MaxValueID : TPhysID read GetMaxValueID write SetMaxValueID;
    property MinValueID : TPhysID read GetMinValueID write SetMinValueID;
...
begin
  ...
  SetMaxValueID(MaxValueID);
  SetMinValueID(MinValueID);
end;
...
procedure TMonometr.LMAXChange(Sender: TObject);
var
  err: boolean;
begin
  inherited;
  if TryStrToFloat(LMAX.Value, F_MaxValue) then
  begin
...
procedure TMonometr.LMAXDblClick(Sender: TObject);
var
  v  : string;
  dt : TDateTime;
begin
  ...
  v := LMax.Value;
  if InputQuery('”кажите максимальное значение давлени€',
    '”кажите максимальное значение давлени€', v) then
  begin
    dt:=0;
    v := StringReplace(v,'.',',',[]);
...
procedure TMonometr.LMINChange(Sender: TObject);
var
  err: boolean;
begin
  inherited;
  if TryStrToFloat(LMIN.Value, F_MInValue)
  then begin
...
procedure TMonometr.LMINDblClick(Sender: TObject);
var
  v  : string;
  dt : TDateTime;
begin
  ...
  v := LMin.Value;
  if InputQuery('”кажите минимальное значение давлени€',
    '”кажите минимальное значение давлени€', v) then
  begin
    dt := 0;
    v := StringReplace(v,'.',',',[]);
...
function TMonometr.GetMaxValueID: TPhysID;
begin
  Result := LMAX.PhysID;
end;

function TMonometr.GetMinValueID: TPhysID;
begin
  Result := LMIN.PhysID;
end;

procedure TMonometr.SetMaxValueID(const Value: TPhysID);
begin
  LMAX.PhysID := Value;
  LValueChange(Self);
  LMAXChange(Self);
...
procedure TMonometr.SetMinValueID(const Value: TPhysID);
begin
  LMIN.PhysID := Value;
  LValueChange(Self);
  LMINChange(Self);
